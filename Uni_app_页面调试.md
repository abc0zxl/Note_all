...
2.**uniapp官网->插件->插件市场连接**

3.**这些插件文件如何使用？**

* 看下载链接的文档说明
* 依赖过期的跟着豆包改
* 这些插件的功能很强，想修改某些地方，它大部分直接用属性封装好了，直接传入对应的属性即可完成修改。

### SKU优化

**问题**：

* 从**规格选项**进入sku时，底部会有两个按钮（购物车，购买），**点击购物车，购买**也是两个按钮，**要修改为一个按钮**

**解决**：

* 会有专门的属性来**指定显示某个按钮**，
* 用**点击事件，传入是什么按钮，然后在事件中修改属性值**
...
![image.png](/assets/540409aa-a5dd-4e2d-bbbc-b5f641a6a455.png)

### 修改收货地址

1.这里用**store**的方法，存储在地址列表页面存储的**选中的地址**
...
4.**在订单详情页面获取store中的地址**，

* 写一个双选，如果store没获取到就用默认地址，因为是computed事件，所以每次修改都会肯定会判断

5.**优化地址列表**：由于设置了点击事件，所以在点击修改的时候也会触发返回上一页而覆盖了修改的方法，只需**在修改地段的标签上**设置一个忽略点击事件方法即可

![image.png](/assets/31a6e897-a188-40ba-a2aa-2ed9ca8b44cb.png)
...
2.**想控制当前页面的各个容器的话就需要获取到当前页面的实例**

* **关键字：** pages.at(-1)
* **设置动画绑定时机**：在页面渲染完毕时，绑定动画效果**onReady（）**
* **设置动画效果**：**pageInstance.animate({**

## 页面栈

每个页面都有一个页面栈，这个表示跳转的深度

### **页面栈与跳转深度**

- **页面栈**：在 uni-app 中，每个页面跳转都会形成一个页面栈，用于记录用户访问页面的顺序和层级。
- **跳转深度**：表示从当前页面返回到目标页面需要经过的页面数量，通常用于控制返回逻辑
...
这个就是替代掉原来的顶部导航栏，换成自己定义的导航栏，但是导航逻辑等都得自己设置
...
* 在显示事件的位置用uni-Countdown标签
* 配置属性信息：
* 配置倒计时秒：**如果有更小单位的数据，大单位的就可以不用写，例如已经知道秒数就不用再配置分钟和小时**
* **绑定超时事件**：
* 修改订单状态
...
1.这个逻辑有一个要点就是查看当前订单是否处于**待收货**的状态，如果是的话才能点击收获，

2.**这个需要在后台修改订单状态**，

## 订单物流

订单仅在**待收货，待评价，已完成**的状态下才会**获取到物流信息**

1.**订单的物流他是主动更新的**：

## 订单详情的游标效果

![image.png](/assets/94f64576-3ff1-44d8-bd95-04a6210bbd09.png)
...
* 这里用get就好
* 所以**将参数拼接到地址上，然后用url传递过去**
* **接收方**：用defineProps，按照对应的名字接收即可
...
* **分包**；当项目页面过多时，未来减小主包体积，要把非核心页面放到**分包**中，
* **分包根目录**：就是各个主包下的二级文件夹路径
* **root**：分包文件夹名字，如果时pages里的，需要加上pages/

4.**preloadRule数组**：预加载设置

5.**easycom**：组件自动引入规则

6.**globalStyle**：全局阳市配置
