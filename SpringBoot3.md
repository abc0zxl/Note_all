...
1.**@AllArgsConstructor**：他可以自动给所有成员生成一个全参构造器。他只负责给成员生成构造器。

* 他会自动将类中的成员变量生成一个构造器
* 然后spring会自动识别到这个构造器，并将spring中所有的bean，匹配注入进来。
* 这样就在创建对向 的时候u就完成了对对象的注入。

2.**@Autowired**：他比较适应老版本，需要在每一层都注入，自动注入，都要写。对象创建后给她赋值。

* 这个

3.**成员变量**：就是在类内部，方法外部所有的变量，就像一个已知的工具

4.**final**：表示这个成员一旦加了这个final，就再也不能改了，防止成员突然被改为空的情况方生。

5.**构造器注入**：是由spring自动识别到构造器，在创建对象时并注入spring管理的对象，@AllArgsConstructor不负责注入。

6.**默认构造器注入**：如果一个bean类，只有一个构造器，不管是手动的还是通过AllArgsConstructor创建的，spring会默认的将这个构造器作为注入入口，会识别构造器的参数，斌找到合适的bean，完全不需要如何注释。

7.**@AutoWired注入原理**：当遇到一个接口多个方法的时候的注入方法。

* **对象名注入**：通过创建类对象的时候的名字来判断。一般就是要注入的那个实现类。**可以辅助spring快速注入**
* **手动声明注入**：再@Autowired下方加入@Qulifier（”AAA“），声明要注入的实现类AAA
* **设定默认的注入对象**：再实现类方法上面写一个注解**@Primary**。

8.**自动注入误区**：通过我在类中调用的某些实现类特定的方法，反过来确定要注入那个bean的

9.**多参数列表，转换到底参数列表**

![image.png](/assets/d5406fa0-587f-4af8-9a86-920432e55445.png)
