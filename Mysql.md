...
1.**MySQL 报 “某一行附近错了，99% 是它上一行少了逗号！**

2.**索引**：一个表可以设定好几个索引方式，但是在查询数据库表的时候可以不用他的名称，这是因为数据库自动识别并自动调用索引

MySQL 有一个叫做 **"查询优化器"（Query Optimizer）** 的组件，它会：
...
![image.png](/assets/34514649-d3d0-4234-b2b6-124d4cb37a44.png)
...
WHERE TABLE_SCHEMA = 'up_door'
AND TABLE_NAME = 'tb_cart';
...
**定义**：mysql支持json类型的属性，可以存入json

**场景**：在商品的规格处理上比较合适，例如一个商品又多个规格，组合选择出来这个具体的商品，然后显示价格，但是每个商品的规格数量都不同，造成原始的id匹配方式内存耗费过多或者查找困难。所以有一个json类型，在json中存放不同数量的类型id，然后查找出来具体的商品。
