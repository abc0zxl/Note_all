...
10.**第一个页面**：是pages.json中的第一个元素，小程序启动会默认用这个
...
* 配置**全局样式，全局声明周期onLaunch，onShow，onHide**
* 负责承载全局布局
* 所以他被称为组件树的根
* 用于关联所有子组件
* **不设置**：会导致全局样式无法生效，生命周期无法监听，手机端应用无法启动。
...
20.**pnpm-lock.yaml和package-lock.json**（货物清单）：依赖包和版本锁定的文件

* 确保在自己电脑上跑的项目，到别的地方他的配置也是相同的也能跑

21.**composables**：组合式函数，是封装各种方法的对方

* **.gitkeep文件**：空文件占位文件，git默认不会提交空的目录，所以可以用这个来让git提交这个空目录。
* **index.ts文件，useGuessList.ts**：统一导出所有composables函数的文件。方便再里面映入那个特别长的引入文件,这里专门用于**上拉加载的逻辑**

22.**dist**（编译输出目录）：这个生成小程序的时候自动生成的，不设置会导致，编译后找不到输出文件，无法发布。

23.**node_modules**（依赖包目录）：这个也是自动生成的，他是存放项目依赖的对方，没有依赖，运行会报错。

24.**uniCloud-aliyun**(云服务目录)：这个是阿里云数据库，的核心，不设置的话无法使用云端服务，按需要搭建。

25.**vite.config.ts**(Vite配置文件)：配置Vite构建规则，这个如果是vue3的过就是用Vite启动的，就得有，不配置的，无法配置路径别名，无法配置代理，手机端调试的时候跨域请求失败，
...
26.**componennts**（可复用Vue组件）：如下列文件

* **styles文件**：里面方的是各种.scss文件，将某些组件的样式单独抽取出来单独维护。不抽取出来的话一个页面中多个组件样式混在一起及其难维护。还容易引发冲突
* **vk-data-goods-sku-popup文件**：他是一个商品页面中点击加入购物车和立即购买的时候的半弹窗，用于选择商品规格。这个组件复用最好，背后的逻辑复杂（库存联动，价格联动，规格联动）
* **vk-data-input-number-box文件**：他是一个数字输入框，为了统一全局中的数字输入框样式。就是用来调整商品数量的的加减按钮和输入数字的框。蛋蛋用现成的input很麻烦，而且会造成组件混乱。
* **components.d.ts文件**：组件类型声明，不写的话引入这个组件的时候没有声明。就需要自己手动导入这个组件。
* **XtxGuess.vue文件**：这个是一个在首页中的**猜你喜欢的组件（小兔鲜）**
* **XtxSwiper.vue文件**：这个是一个**轮播图组件**

27.**pagesMember页面分包**：由于文件超过2M，就会导致git无法上传。以及包过大，导致加载卡顿的情况。可以用来存放一些非导航的页面，如设置，个人中心，地址页面等。

28.**utils文件**：存放工具的地方，如http请求头的设置工具

* **拦截器**：如发送请求的时候检查是否登录等。然后执行跳转

29.**gitignore文件**：语法

* **'*'.AAA**：匹配所有.log结尾的文件
* **AAA**：直接写文件名字也可以
* **dist/**：忽略**根**目录下的所有dist目录及其文件，**注意是根目录下**，不加斜杠的话，会删除所有目录中名字为dist的**目录和文件**
* **#**：这个是注释

30.**project.config.json**：这个更多是给微信小程序端做细粒度的配置补充，如果uniapp项目需要做微信小程序则必须要用它

* 定义代码压缩，语法转换规则
* 撇脂特殊的npm包打包规则
* 设置小程序端独有的变一选项

31.**project.private.config.json**：用于**项目迁移，团队协作，版本升级**的时候用的文件。
...
2.**新建云数据库**：搜索要新建的数据库表，（中国城市）

**配置HBuilder**

3.**选中项目**：鼠标右键选择unicloud**阿里云**，到文件夹中的unicloud再右键关联即可

4.**点击到database文件夹中**：右键点击下载data schame，下载数据库配置，**右键格式化**

![image.png](/assets/20599bde-b4d3-4261-b28d-3a71fdde6cdb.png)

## 城市选择的实现

再每个端之间所用的标签不同，而且每个标签的实现功能都不同

![image.png](/assets/2e112d1d-5c80-4140-9c52-29387bedee4a.png)

**如上图所示，两种标签的实现的效果也不一样**

1.**问题展示**：H5上，他不识别picker

![image.png](/assets/5315ba68-981a-4b9f-9c8d-8ad9e0e9299d.png)

![image.png](/assets/e5ee8753-387e-415d-b887-842c96908406.png)
...
![image.png](/assets/aea53ae0-25cb-4878-9592-a64f10926eed.png)

3.**再测试阶段这个需要用HBuilder来运行才能作用**

## 地址在H5端样式的兼容问题

1.**优先去浏览器中的控制台找**：找到样式区域，单独修改

2.**调用class，修改直接它的样式，要配合上条件编译**

3.**这种新添加的H5端地区表**：也需要单独编写一套获取它的参数的逻辑

* 也是要用到**条件编译**

![image.png](/assets/b1820f6d-ed8d-4d78-a530-ade8323d5d55.png)

* 还要调整一下细节：由于原来的**选择地址时**的**点击事件**的**屏蔽事件**它只适配于微信小程序端，所以也要针对H5端的，**屏蔽事件**做一个增加
...
核心作用是让前端开发者无需搭建传统的后端服务器，就能直接使用云端资源完成应用给的后端功能开发。

1.**逻辑编写**：

用熟悉的js写云函数，就能拿在云端运行逻辑，无需担心服务器运维。

2.**它提供云端数据库**：

支持json格式数据存储，和视频，图片的上传和下载，前端你可直接调用api操作

3.**用户体系管理**：

它内置完整的**用户登录系统**，支持手机号**一键登录，微信登录，实名认证**等功能，开发者无需从零搭建用户系统。

**总结**：无论是uniapp转App的应用跳转微信登录还是微信小程序一键快捷登录都可以用自己的后端来实现。**id_uni_common不是必须的**

# uniapp中的jwt验证

**问题**：我在**小兔鲜**项目中看到了两套jwt验证的代码

* 在http工具中用拦截器中提取状态，在http的head中写入token
* **另一个**：官方发提供的一个jwt验证文件**uni_id_common**

1.**这两个的作用**：

* 第一个是将http携带上token然后发送给后端来验证，就是平常的验证，然后返回状态码
* **另一个**：复杂在后端，云函数中**生成、校验、刷新、管理token**
* 这两个都是一个作用
...
1.**使用方法**：直接在页面script中写入uni.chooseLocation(success(res))

2.**直接用一个方法按钮触发上面的组件**

3.**还需要再配置中导入**

![image.png](/assets/2521b89f-0f49-4d4c-968e-973bb87caab4.png)
