...
* **为什么不用注解？而要自己编写**：因为这个是向外部发送请求，**外部场景复杂多变**，注解便不再合适，就需要自己编写逻辑
...
用户完全可能同时发起多个请求（比如浏览器开两个标签页同时操作），会产生多个线程，但 ThreadLocal 天然能帮你 “分清” 每个线程的用户 ID，**不会串号**

![image.png](/assets/c3ecd88e-e6b6-4621-a154-04681449f8cf.png)

## 分页插件

分页插件+MyBatis-Plus：**在sql映射文件的应用场景下**，可以便捷的**实现分页**

1.**添加插件**：

在全局文件中添加插件，**注意要加@Bean**
