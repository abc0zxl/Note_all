...
17. **实例变量**就是**成员变量**或者**对象变量**
18. **静态变量**就是**类变量**
19. 所有**守护线程**都是**用户线程**结束后**强制结束**
...
3.**缺点**：获取返回值时，会阻塞当前线程，**效率低**

##### 步骤

1.创建一个**未来任务类对象**，FutureTask是一个**任务容器**，他的实例化对象是callable

例子：**FutureTask** task=new FutureTask(new Callable())

2.创建线程，他的**实例化对象**就是这个未来任务对象
...
可以将多个线程的共享的对象放入到一个对象中，每当线程访问这个对象时就会上锁，别的线程访问不了，知道使用完毕。实现线程的同步。

**对象锁：** 用于实现共享资源的同步，防止冲突。线程对象访问这个共享对象时

**核心语句**：`synchronized(this)`这个this最重要，`this` 代表当前类的实例对象，每个实例对应唯一的一把锁；
...
**StringBuilder** 适用于安全的局部变量，他的效率比较高

**StringBuffer** 适用于不安全的共享变量，但是它的效率比较低

#### java三大变量

**实例：堆** ，因为是共享的，所以存在安全问题

**静态：方法区** ，因为是共享的，所以存在安全问题

**局部：栈**，不存在安全问题

synchronized：同步，对象锁，类锁

#### 解决线程安全的问题

1.用**局部变量** 代替**实例变量**和**静态变量**这种共享变量

2.不用共享对象，每个**线程**一个对象，这样实例变量的内存就不共享了

3.以上两个都不行的话就用synchronized
...
设置为守护线程后，他会一直执行，不会耽误**主线程**的进度，**主线程**继续执行下面的语句，直到主线程结束，然后**守护线程**会被**强制打断**
...
所以设置了一个变量来检测是否是，正常的恢复阻塞。再配合while循环达到一直检测的效果。
