...
是一种全双工通信协议，建立在TCP上，可以让客户和服务器在同一个链接上互动发消息，而不是轮询。

1.他不同于http发送连接：http是请求+响应+结束

* 他是客户端和服务器之间建立一个持久的双向连接，可随时收发。

## 实现过程

1.**后端暴露端点**：

* @ServerEndpoint声明这个端点，也是一个可被访问的路径
* 后端接到，会话id和会话内容，建立连接请求
* **客户端建立连接**：后端记录这个请求，
* **客户发送消息时**：需要接收消息内容和会话id，
* **消息发给客户端**：

2.**客户端建立连接**

* 发送连接连接URL，并附上id，**建立连接**
* 连接成功回调：**连接成功消息**
* **收到消息**：这个是一个回调，后端发消息过来，就会自动执行
* **发送消息也可以**
* 如果没有刷新页面好像会话会一直保留：关闭连接

3.**后端如何分辨谁发过来的消息**：

* 通过每次前端建立连接：携带在url上的会话id
* 服务器也会通过get进去对应的sessionId创建对象，给对应的客户发送消息。
...
4.消息进入到了消息界面中，this.message.push

5.发送消息。this.ws.send(cotent)
